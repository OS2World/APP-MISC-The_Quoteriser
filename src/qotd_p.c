 ...
 * qotd_p.c
 *
 * Window painting routines for the Quoteriser's Quote-of-the-Day.
 *
 ...
 */
{
	...
	/* go */
	WinPostMsg(pti->hwndCaller, QM_START, NULL, NULL);
 ...
 * QM_COPY	- copy then current display to the clipboard
 */
{
	...
	switch (message) {
		...
		case QM_TYPESET:
			...
			WinSendMsg(hwndVertScroll, SBM_SETPOS, MPFROMSHORT(0), 0);
			return (FALSE);
